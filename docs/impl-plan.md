...
Remaining (needs game host): the pre-planned live session (steam-re.md "Next live session") — verify + confirm the four new patch families, AI-pred fire-rate, and the fire_fix overlap observation. Remaining code work (no game): client UI polish (design-level), OP_SET_NAME FO3/Steam field reconciliation (live probe), OP_PLACE_AT_ME (by design unused). CI note: the i686 cross-check needs ~/.cargo/bin/cargo (rustup) — the Fedora system cargo doesn't see rustup targets.

| 27. Challenge pass (2026-08-18i) | Re-examined every "needs-live" item with sharper tools (CallGraph.java BFS + targeted marker fingerprints). **ai_fix4 WIRED** — the GOG NOP'd call = the spawn-init (distinctive [actor+0x60] vtable+0x464 marker); Steam twin 0x734930 found by fingerprint → creation fn 0x63FA90 (twin of 0x42FB60) → call at 0x63FB36 (13-byte block, byte-verified) → NOP'd. **fire_fix PROVEN redundant** — the delegator reaches both the fire-dispatch loop and the wired per-frame executor, so wiring fire_fix double-reports command fires; stays unwired (proof, not hedge). **OP_SET_NAME field conflict discovered** — OP_GET_NAME is never sent (get_name unexercised); the engine's GOG SetActorFullName writes [base_form+0xD4] vs the bridge's +0x1C read; FNV consistent, FO3/Steam suspect. **AI-pred fire-rate structurally high** — all 12 callers are per-frame processing fns. New tool: CallGraph.java. 605 tests. | bridge `hooks/vaultmp.rs`, scripts/re/ghidra/CallGraph.java, docs/steam-re.md 18i |

Remaining (needs game host): the pre-planned live session — verify the wired patches (Steam kill/match_race/ai_fix4, FNV lock, play_sound), confirm the fire-rate, reconcile the FO3/Steam name field. Remaining code work (no game): play_group/play_idle delegator relays (vaultmp-legacy, low priority), client UI polish, more game modes. CI note: the i686 cross-check needs ~/.cargo/bin/cargo (rustup).
