...
Remaining (needs game host — steam-re.md): **live 2026-08-15 —** Steam discovery detour 0x7F9B70 is a prologue false-positive (installs byte-perfect but ~never fires) and the Steam frame hook 0x9B3D77 never fires — both need re-derived Steam sites (classic 0x6FAE90 + FNV ActorProcessManager stay as-is). Remaining Steam patch-site groups (fire_fix, match_race, place_at_me, ai_fix2/3/4 — still need live-probe) and the Steam AV/anim vtable slots (GetActorValue/State/is_moving — the game's real AV access is via the ActorValueOwner sub-object at Actor+0x9C (FO3) / +0xA4 (FNV), confirmed from the engine's command-table handlers 2026-08-14; the Steam equivalents need OP_PROBE_FORM). Live-verified 2026-08-15: fire_weapon 0x770880, get_activate 0x8D3BC8/0x8D3CB8, ai_fix1 0x5E99E2, play_idle_fix (4 cands), play_group_fix 0x4350F9, delegator pad 0x405E69. AV getters/setters + kill_actor wired for classic/FNV (command-table-derived). Remaining bridge OP stubs: OP_SET_NAME (needs GetFullName — a non-virtual TESForm fn, currently "unnamed"), OP_PLAY_SOUND (intricate SoundManager flow both builds), OP_PLACE_AT_ME (unused). Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games. 3D client view dropped from scope (game window is the view; client's top-down + health bars/names + server GUI is the companion HUD). Client HUD shows local player health (AV getters wired); freeroam gained !pvp/!time/!weather/!resurrect commands with e2e. 600 tests.
