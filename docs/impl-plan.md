...
Remaining (needs game host — steam-re.md): **live session** — verify the 0x7D0A50 discovery fire-rate; confirm semantics of the 113 restructured Steam vtable methods only if a specific one is needed (identification is complete — vtable_twins.txt); wire fire_fix relay stub (needs 5-byte E9 + relocated re-entry in a code cave — layout derived, wiring deferred); play_group delegator flow (entry byte unpinnable); play_idle_fix wiring (twin chosen: handler site 0x79F2BB, delegator relay machinery pending); pin ai_fix4 (0 byte anchors). Remaining bridge OP stubs: OP_SET_NAME (FNV engine setter 0x489100 found but this-semantics partial — FO3/Steam setters not derived), OP_PLAY_SOUND (intricate SoundManager flow both builds), OP_PLACE_AT_ME (unused). FNV lock getter confirmed (0x57B410 @ slot 0xB8) but FNV lock setter unknown — set_lock stays FNV-no-op. Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games. 3D client view dropped from scope (game window is the view; client's top-down + health bars/names + server GUI is the companion HUD). Client HUD shows local player health (AV getters wired); freeroam gained !pvp/!time/!weather/!resurrect commands with e2e. 601 tests.
