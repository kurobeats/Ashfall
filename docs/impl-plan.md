...
| 12. FNV mapping + frame hook | FNV static session: PlayerCharacter singleton 0x011DEA3C (NVSE), main-loop frame hook 0x86B386 (NVSE kMainLoopHookPatchAddr — `call 0x43C4B0` before Sleep(50), wired as `apply_fnv_frame_hook()`: byte-guarded redirect → original getter + 10 Hz `report_player_state_due`), HighProcess 0x8EEEC0 identified. No Steam/GOG split for FNV (single 1.4.0.525 build). FNV AI predicate (discovery detour) still open — see steam-re.md. | bridge `vaultmp.rs`, docs/steam-re.md |
