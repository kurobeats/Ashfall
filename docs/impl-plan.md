...
Remaining (needs game host — steam-re.md): live verification of the discovery detours (GOG 0x6FAE90 + Steam 0x7F9B70 — probe-verify before patching, FLAT/+0xC00 caveat), the per-frame game-loop hooks, and the remaining Steam patch-site groups (get_activate_ret final, fire_fix, match_race, place_at_me, ai_fix2/3/4, play_idle_fix — all need live-probe). Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games.
