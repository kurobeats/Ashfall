...
| 21. Test-coverage sweep (2026-08-14i) | Zero-test modules eliminated: master ServerList (upsert/cull), client IPC wire (opcodes, Param layout, CommandResult decode), bridge network framing (pipe roundtrip, tracked-actor set, 10 Hz throttle), combat formula + resolver (DR/DT, crit, range, death), physics/item/chat/actor/player/combat handlers (ownership + anti-cheat + PvP gate), QuestManager/WeatherState/GlobalState/FactionMatrix (clone-share semantics), CellGrid/CellContext, AI packages, Session, ObjectRegistry, world objects, core math/FormID/NetworkID/ObjectKind/Channel. 486 → 600 tests. | all crates |
| 22. Windows client build (2026-08-14j) | The native client cross-compiles for x86_64-pc-windows-gnu: IPC transport's Unix-socket mode is `#[cfg(unix)]`-gated (Windows uses TCP loopback to the Proton bridge), `game.rs` maps an "unix" config to TCP on non-Unix. New `cross-windows-client` CI job. Runtime still needs a Windows host to verify. | client `ipc/transport.rs`, `game.rs`, CI |
| 23. Live session + i686 bridge fixes (2026-08-15) | First live Steam-build session (game host tetsuo.chaotic.lan). Fixed three blockers: (1) i686 bridge never actually linked — `global_asm!` symbols lacked the C-ABI `_` prefix (`cargo check` only type-checks); renamed all asm symbols + extern decls. (2) thiscall/vcall inline-asm shims over-constrained registers on i686 — rewrote to `in("ecx") this` + `push {arg}` + `call {addr}` + `lateout("eax") ret`. (3) TCP server dropped clients after 50ms idle (`ErrorKind::TimedOut` treated as fatal) — now matches `WouldBlock | TimedOut`. Live-verified (read-only probes): fire_weapon 0x770880, get_activate 0x8D3BC8/0x8D3CB8, ai_fix1 0x5E99E2, play_idle_fix (4 cands), play_group_fix 0x4350F9, delegator pad 0x405E69, respawn patch behavior. Found the Steam discovery detour (0x7F9B70) + frame hook (0x9B3D77) install but ~never fire — re-derivation needed. 600 tests still green. | bridge `hooks/address.rs`/`vtable.rs`/`vaultmp.rs`/`network.rs`, docs/steam-re.md |
| 24. Static RE campaign (2026-08-17) | data/re campaign (lanes A1/A2/A3/B1/B2 + validation 27/27 + C1–C4). **Steam FO3 command table found** (0x110B388, 569 entries, format A) → 13 handlers → engine fns (Kill 0x7F3200, Resurrect 0x8C2B30, MoveTo 0x79BC20, PlaySound flow, ForceActorValue → avOwner+0x9C/slot3/SetAV 0x3A0). **Corrected Steam AI predicate 0x7D0A50** (0x7DAF80 was a false-positive twin — discovery never fired). Corrected anim-data slot 0x1E4→0x1EC (Steam 0x244). FNV: command table 0x1190950 (569 entries, dumped — scripts/re/tables/fnv_cmdtable_0x1190950.txt), handler inventory, class vtables, Kill 0x8B86E0 signature pinned (4-arg). **Wired**: `apply_steam_vaultmp()` (ai_fix2/3, delegator chain, place_at_me, fire_weapon 0x7DF3F7), FNV kill_actor (0x8B86E0), set_lock_flags FNV no-op guard (FNV lock is a different mechanism — +0x20→slot 0xB8). 31 suites still green. | bridge `hooks/vaultmp.rs`/`mod.rs`/`vtable.rs`, docs/steam-re.md, data/re/ |
| 25. Ghidra validation + decompile campaign (2026-08-18) | Ghidra 12.1.2 headless on battlecruiser (no game). **(a) Full validation**: every bridge address table byte-verified on GOG/Steam/FNV (all byte-guards held; GOG LookupFormByID 880 refs + AI predicate 11 callers; Steam cmdtable 13/13 handlers + AI predicate exactly 12 callers; FNV cmdtable fully mapped → 569 entries, KillActor 0x5C7F10, ForceActorValue 0x5CD910, ModPCSkill 0x5BE190). Errors corrected (docs/comments): GOG Actor vtable base 0xE16B10 → **0xE18110**; Steam respawn flag-write 0x8CA8EB → 0x8C9D52; FNV MoveTo 0x79BC20 (Steam's — FNV = padding, real 0x79BC50); cmdtable names are POINTERS. **(b) Decompile pass**: **Steam kill_actor wired** (0x7F3200 = twin of GOG 0x71C280; the former "unmapped [ebp+0xC]" arg = limb; death processor 0x7D4F40 = 8-arg matching GOG); **Steam match_race wired** (jne NOP @ 0x6F722C — +0x110 race-id compare); play_idle twin choice resolved (0x79F2BB = the PlayIdle command handler); AI predicate 0x7D0A50 decompiler-confirmed 1:1 twin; FNV lock getter confirmed (0x57B410 @ slot 0xB8 family) + FNV name-setter 0x489100 found. **(c) Vtable slot map RECOVERED** (VTableSlots.java): decompile-twinning (normalized-C SHA1 + Jaccard) → 147/260 Steam real fns have GOG twins (50 @ J=1.00), the other 113 position-identified (restructured methods), **0 true unknowns** — the "~310 slots need OP_PROBE_FORM" claim is obsolete. Artifacts: scripts/re/ghidra/ (VerifyRE.java, DecompileFns.java, VTableSlots.java, vtable_*.txt, decomp_*.txt), scripts/re/tables/ (3 cmdtable dumps). 605 tests. | bridge `hooks/mod.rs`/`vaultmp.rs`, docs/steam-re.md, scripts/re/ghidra/, scripts/re/tables/ |

Remaining (needs game host — steam-re.md): **live session** — verify the 0x7D0A50 discovery fire-rate (structurally high — 11/12 callers are per-frame processing fns, per 2026-08-18i); confirm semantics of the 113 restructured Steam vtable methods only if a specific one is needed (identification is complete — vtable_twins.txt); wire fire_fix relay stub (needs 5-byte E9 + relocated re-entry in a code cave — layout derived, wiring deferred); play_group delegator flow (entry byte unpinnable); play_idle_fix wiring (twin chosen: handler site 0x79F2BB, delegator relay machinery pending); ai_fix4 WIRED 2026-08-18i (Steam site 0x63FB36 found via the spawn-init twin 0x734930's +0x464 marker). Remaining bridge OP stubs: OP_SET_NAME (FNV engine setter 0x489100 resolved — this=form+0x18; needs game-heap name buffer plumbing; FO3/Steam setters not derived), OP_PLAY_SOUND WIRED 2026-08-18e (Steam 0x9CC980 / GOG 0xBCFBB0, cdecl sound_form/refID/flags), OP_PLACE_AT_ME (unused). FNV lock relay WIRED 2026-08-18d (state = [obj+0x3C] bit 1, engine setter 0x60CA30 — Lock/UnLock handlers 0x5C7280/0x5CBF80). Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games. 3D client view dropped from scope (game window is the view; client's top-down + health bars/names + server GUI is the companion HUD). Client HUD shows local player health (AV getters wired); freeroam gained !pvp/!time/!weather/!resurrect commands with e2e. 605 tests.

| 17. Steam vtable re-derivation (2026-08-14c) | Steam PC vtable base found (0xF938FC, verified via AI-pred +0x22C and death-handler +0x23C); GOG base 0xE16B10 (CORRECTED 2026-08-18 → 0xE18110 — the true Actor vtable; 0xE16B10 was a different class's run-start). Slot translation by byte-identical matching: 41 slots, 59% fit a +0x58 shift (recompile inserted 22 early entries). Early region (+0x00..0x68) REORDERED — GetActorValue/BaseValue/AnimData need live probe (mostly resolved 2026-08-18 by decompile-twinning — see row 25). GET_LOCKED confirmed: GOG +0xA0 -> Steam +0xFC (byte-identical `8a 41 0a 24 01 c3`). Wired `fo3_steam_vtable` + `steam_slot_for()` into `get_lock` (byte-guarded, GOG fallback). Online: no public Anniversary vtable exists (kFOSE checked — classic only). | bridge `vtable.rs`, scripts/re/vtable_steam.py, docs/steam-re.md |
...
| 26. Game-free completion pass (2026-08-18) | Everything statically derivable is wired. **(a) OP_SET_NAME wired (FNV)** — engine SetFullName 0x489100(base_form+0x18, name), writes the exact field get_name reads; FO3/Steam assigners identified (0x4019C0 / 0x57D590) but handler field-resolution needs a live name-change probe. **(b) scripts/shared-quest/** — second WASM game mode: server-wide shared quest (NPC kills count together, progress broadcast, stage advance, player deaths don't count); real-wasm integration test (skips if unbuilt). **(c) reliability edge cases** — send-window-full throttle (MAX_INFLIGHT 32), near-1200B payload roundtrip, reliable/unreliable interleave on one session. **(d) bridge_probe `verify` action** — 24 wired patch sites byte-checked + build auto-detect; pre-planned live session (AI-pred fire-rate, Steam kill, match_race, FNV lock, play_sound, fire_fix overlap). OP_PLACE_AT_ME left stub by design (spawns are owner-side via OwnershipClaim). fire_fix stub recipe complete but wiring gated on the live overlap observation (discipline: risky code patches wait). 605 tests, clippy/fmt clean, i686 cross-check clean. | bridge `hooks/mod.rs`/`commands.rs`, scripts/shared-quest/, scripts/README.md, crates/ashfall-server/tests/{reliability,script_runtime}.rs, scripts/re/bridge_probe.py, docs/steam-re.md |

| 28. vaultmp source derivation (2026-08-18j) | All "vaultmp-legacy" deferrals derived from the vaultmp source (/home/ants/dev/vaultmp-extended, MIT): the delegator chain = cross-thread command dispatch (pipe thread queues → game thread re-dispatches via the hooked delegator, Sleep(1) loop). **ai_fix4 rationale found** — git commit c8141bb "Fix 'radiation suit equipment bug' close #96" (2013): NOP'd the spawn-init call whose player-singleton checks misfired on remote equipment; the Ghidra-derived WHAT (0x559740/0x734930) now has its WHY. **play_group/play_idle** = vaultmp's event-based anim relay (idle anim capture at [ECX+0x414] + group-command forwarding) — proven redundant with Ashfall's state-based ActorStateDelta anim sync. **fire_fix** = shooter-verify relay — redundant (call-graph proof, 18i). All legacy items now wired (ai_fix4), proven redundant, or documented with original rationale. 605 tests. | docs/steam-re.md 18j |

Remaining (needs game host): the pre-planned live session — verify the wired patches (Steam kill/match_race/ai_fix4, FNV lock, play_sound), confirm the fire-rate, reconcile the FO3/Steam name field. Remaining code work (no game): client UI polish, more game modes. CI note: the i686 cross-check needs ~/.cargo/bin/cargo (rustup).

| 29. Live session #2 (2026-08-18k, tetsuo) | First real results on the Steam build. **Discovery 0x7D0A50 PASS** — 14 NPC_SPAWN + 11 NPC_REMOVE in 40s (the re-derived predicate is the true per-actor gate). **Name field RESOLVED** — the player's actual name lives at [base_form+0xD4], not +0x1C (live-probed "Anthony"); get_name fixed per-build (+0xD4 FO3/Steam, +0x1C FNV) and live-verified ("Anthony"/"Nathan"). **match_race behavioral PASS** — placeatme Lucas Simms, correct body. Three wired-but-never-exercised hooks crashed the game live: delegator chain (11-byte Steam site vs 5-byte relcall → AV 0x9B3F09), fire_weapon reljumphook (garbage eax → [eax+0xC] fault on any gunshot), play_sound 0x9CC980 (wrong convention). Removed/deferred. Server hardening: catch_unwind per client + game-thread engine-call queue drained in the discovery collector (the frame hook 0x9B3D77 emits zero events). 605 tests. | bridge hooks/{vaultmp,mod}.rs + network.rs, docs/steam-re.md 18k |

| 30. Kill/sound direct-call dead ends (2026-08-18l, Ghidra + live) | **Kill**: decompiled KillActor handler 0x798800 — the death processor 0x7D4F40 dispatches on the limb-BIT (0x19-0x1F sync dismembered death; other values = an async death-request queue that never processes bridge-made records); the handler's death-setup 0x7E53D0 crashes outside the command context (live-verified). vtable+0x1D0 = 0x8B8AF0 (live-probed; RTTI + twin heuristic maps both wrong). **PlaySound**: 0x9CC980 = SEH wrapper (out_buffer_12B, sound_ref_id, flags); out-buffer fix still crashes on the play call 0x9CC220. Both reverted to safe no-ops. Delegator identity corrected (0x9B0740 real; 0x405E70 = init helper). Verdict: kill + play_sound need the vaultmp delegator relay (dispatch op 0x108B / 0x1026 through the engine's command executor on the game thread) — all conventions mapped. 605 tests, clippy/fmt clean, i686 cross-check clean. | bridge hooks/mod.rs, docs/steam-re.md 18l, scripts/re/ghidra/dec_kill_relay*.txt |
