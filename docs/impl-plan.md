...
396 tests, 0 warnings. Ownership/delta rules proven in `tests/ownership.rs` (5 handler-level tests); string-cache wire semantics in `string_cache.rs` unit tests + `wire_format.rs`. Next: bridge `events.rs` NPC-spawn reporting → `claim_ownership()` wiring.
...
Remaining (needs game host RE — steam-re.md): the per-frame game-loop hook that calls `report_player_state()`, and loaded-cell NPC enumeration → `EVENT_NPC_SPAWN` (the ownership loop's NPC half). Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games.
