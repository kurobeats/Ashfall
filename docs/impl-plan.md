...
Remaining (needs game host — steam-re.md): live verification of the discovery detours (GOG 0x6FAE90 + Steam 0x7F9B70 — probe-verify before patching, FLAT/+0xC00 caveat), the remaining Steam patch-site groups (fire_fix, match_race, place_at_me, ai_fix2/3/4, play_idle_fix — all need live-probe), the Steam AV/anim vtable slots (GetActorValue/State/is_moving — the game's real AV access is via the ActorValueOwner sub-object at Actor+0x9C (FO3) / +0xA4 (FNV), confirmed from the engine's command-table handlers 2026-08-14; the Steam equivalents need OP_PROBE_FORM), and behavior-verifying the mapped sites + hooks in-game. get_activate fully solved (jmp 0x8D3BC8 + ret 0x8D3CB8). Per-frame player hooks wired for ALL builds (FNV 0x86B386, FO3 classic 0x6EEB2F, Steam 0x9B3D77). AV getters/setters + kill_actor wired for classic/FNV (command-table-derived). Remaining bridge OP stubs: OP_SET_NAME (needs GetFullName — a non-virtual TESForm fn, currently "unnamed"), OP_PLAY_SOUND (intricate SoundManager flow both builds), OP_PLACE_AT_ME (unused). Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games. 3D client view dropped from scope (game window is the view; client's top-down + health bars/names + server GUI is the companion HUD). Client HUD shows local player health (AV getters wired); freeroam gained !pvp/!time/!weather/!resurrect commands with e2e. 600 tests.
