...
| 24. Static RE campaign (2026-08-17) | data/re campaign (lanes A1/A2/A3/B1/B2 + validation 27/27 + C1–C4). **Steam FO3 command table found** (0x110B388, 569 entries, format A) → 13 handlers → engine fns (Kill 0x7F3200, Resurrect 0x8C2B30, MoveTo 0x79BC20, PlaySound flow, ForceActorValue → avOwner+0x9C/slot3/SetAV 0x3A0). **Corrected Steam AI predicate 0x7D0A50** (0x7DAF80 was a false-positive twin — discovery never fired). Corrected anim-data slot 0x1E4→0x1EC (Steam 0x244). FNV: command table 0x1190950 (615 entries), handler inventory, class vtables, Kill 0x8B86E0 signature pinned (4-arg). **Wired**: `apply_steam_vaultmp()` (ai_fix2/3, delegator chain, place_at_me, fire_weapon 0x7DF3F7), FNV kill_actor (0x8B86E0), set_lock_flags FNV no-op guard (FNV lock is a different mechanism — +0x20→slot 0xB8). vtable map static-exhausted (89 byte-matches; ~310 slots need OP_PROBE_FORM). 31 suites still green. | bridge `hooks/vaultmp.rs`/`mod.rs`/`vtable.rs`, docs/steam-re.md, data/re/ |

Remaining (needs game host — steam-re.md): **live session 2026-08-17** — verify the 0x7D0A50 discovery fire-rate (re-wired after the 0x7F9B70/0x7DAF80 false-positive pair), OP_PROBE_FORM the ~310 unmatched Steam vtable slots + anim 0x244/0x248 pins, OP_PROBE_CODE the Steam kill death-path (engine Kill 0x7F3200 mid-arg [ebp+0xC] unmapped + death-processor 0x7D4F40 arg order) to finish Steam kill_actor, re-derive fire_fix relay stub + match_race recipe bytes from live disasm, and pin ai_fix4 (object-creation path fully recompiled — 0 byte anchors). play_idle_fix twin choice (0x79DA88/0x79F2BB — vtable-only refs, no static caller split) + play_group entry-JE byte (candidate 0x580BD0) + play_group_fix_src (source fn inlined) + FNV GetFullName + FNV lock getter (0x57B410 slot 0xB8) all need probes. Remaining bridge OP stubs: OP_SET_NAME (needs GetFullName — a non-virtual TESForm fn, currently "unnamed"), OP_PLAY_SOUND (intricate SoundManager flow both builds), OP_PLACE_AT_ME (unused). Load-order bridge reporting explicitly deprioritized — MVP targets vanilla games. 3D client view dropped from scope (game window is the view; client's top-down + health bars/names + server GUI is the companion HUD). Client HUD shows local player health (AV getters wired); freeroam gained !pvp/!time/!weather/!resurrect commands with e2e. 600 tests.
