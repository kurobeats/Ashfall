...
- **Proton runtime testing** — **big progress on the Steam build** (2026-08-07, live on tetsuo): image dump pipeline works (PE-header based), Steam `LookupFormByID` re-derived at **0x711EF0** (form map 0x1224B84) and auto-selected by prologue signature, **cdecl + thiscall convention bugs found and fixed** (vtable thiscall shims validated under wine). **Live with a loaded save**: `OP_GET_POS` returns real coords `(42878.5, -72844.4, 11118.6)`, angle/cell/field getters work, lookup + vtable probe green. **Blocked on**: vtable-call getters (`get_base`, `get_actor_value`) — the Steam TESObjectREFR vtable layout differs from xFOSE at index 4 (destructor, not GetBaseForm); next step is locating the real GetBaseForm slot / baseForm field via `OP_PROBE_FORM` (obj-field scan for the player base form 0x707). GOG 1.7.0.3 = classic table confirmed byte-for-byte. See [docs/proton-testing.md](./docs/proton-testing.md) + [scripts/re/README.md](./scripts/re/README.md).
