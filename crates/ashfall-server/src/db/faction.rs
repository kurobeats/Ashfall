...
impl super::Database {
    pub fn load_all_factions(&self) -> Vec<FactionRow> {
        let mut stmt = match self
            .conn()
            .prepare("SELECT faction_id, name, hostility_mask FROM factions")
        {
            Ok(s) => s,
            Err(_) => return vec![],
        };
    ...
    pub fn set_faction(&self, faction_id: u32, name: &str, hostility_mask: u32) {
        let _ = self.conn().execute(
            "INSERT OR REPLACE INTO factions VALUES (?1,?2,?3)",
            params![faction_id, name, hostility_mask],
        );
    }
