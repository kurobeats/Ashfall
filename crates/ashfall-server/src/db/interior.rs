...
impl Database {
    ...
    pub fn get_interior(&self, cell_id: u32) -> Option<InteriorRow> {
        ...
        stmt.query_row(params![cell_id], |row| {
            Ok(InteriorRow { cell_id: row.get(0)?, name: row.get(1)? })
        })
        .ok()
    }
    ...
    pub fn insert_interior(&self, cell_id: u32, name: &str) {
        let _ = self
            .conn()
            .execute("INSERT OR REPLACE INTO interiors (cellID, name) VALUES (?1, ?2)", params![cell_id, name]);
    }
    ...
    pub fn load_all_interiors(&self) -> Vec<InteriorRow> {
        let mut stmt = match self
            .conn()
            .prepare("SELECT cellID, name FROM interiors")
        {
            Ok(s) => s,
            Err(_) => return vec![],
        };
