...
async fn test_all_packets_delivered_in_order_under_loss() {
    ...
    while seen.len() < PACKETS {
        ...
        if next_send < PACKETS {
            let packet = Packet::GameChat {
                message: format!("msg {next_send}").into(),
            };
    ...
    assert!(
        attempts < 500,
        "retransmission converged ({attempts} attempts)"
    );
    eprintln!(
        "delivered {}/{} under {}% loss in {attempts} receive attempts",
        PACKETS,
        PACKETS,
        LOSS_RATE * 100.0
    );
